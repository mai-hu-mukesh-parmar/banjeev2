...
export default function RenderCall({ chatContent }) {
  ...
  const renderPill = (image, text) => (
    ...
    >
      <Image style={{ width: 22, height: 22, marginRight: 5 }} source={image} />
      <Text style={{ fontSize: 12, fontWeight: "bold" }}>{text}</Text>
    </View>
  );

  const renderOtherTypes = (data) => {
    const { callType, incomingCall, outgoingCall, callDuration, missedCall } =
      data;
    if (callType === "Video") {
      if (incomingCall) {
        return renderPill(
