...
export default function VideoCall() {
  ...
  const icecandidateHandler = React.useCallback(
    (data) => {
      ...
      console.log("Getting icecandidate");
  ...
  const sendReady = useCallback(() => {
    console.warn("Videocall.js sendReady function called");
    console.warn("Videocall.js sendReady function called", {
      roomId: params.roomId,
  ...
  const createOffer = useCallback(
    (data) => {
      console.warn(
        "Videocall.js createOffer fun",
        systemUserId,
        data?.initiator?.id
      );
      if (systemUserId !== data?.initiator?.id) {
        // if (systemUserId === data.toUserId) {
        console.warn("Videocall.js Offer create socket event emit");
        pc?.createOffer({
  ...
  const callFunc = useCallback(async () => {
    ...
    console.warn("Videocall.js call function called");
    console.warn(
      "Videocall.js call function log",
      systemUserId,
    ...
    //   console.log("ON READY USER A");
    //   createOffer(data);
    // });

    socket.on("OFFER", async (data) => {
      console.log("Videovcall.js onOffer recived");
    ...
    <Fragment>
      {myStream?.getVideoTracks()[0].enabled ? (
        ...
        <View style={{ flex: 1 }}>
          <ImageBackground
            resizeMode="cover"
            source={{ uri: listProfileUrl(params?.targetUser?.id) }}
            blurRadius={2}
      ...
      {systemUserId === params.initiator.id ? (
        ...
        >
          <View style={styles.subView}>
            <View>
              <Text fontSize={"lg"} style={styles.name}>
        ...
        <ImageBackground
          resizeMode="cover"
          source={{ uri: listProfileUrl(params?.initiator?.id) }}
          blurRadius={2}
          style={{ height: "100%", width: "100%" }}
        >
          <View style={styles.subView}>
          ...
          <View style={{ flex: 1 }}>
            {myStream?.getVideoTracks()[0].enabled ? (
              ...
              <FastImage
                ...
                resizeMode={"cover"}
                source={{
                  uri: listProfileUrl(params?.initiator?.id),
                }}
              />
            )}
          </View>
